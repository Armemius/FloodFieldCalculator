...
namespace pwn::ffc::core {
  BowtieParabolicFilter::BowtieParabolicFilter(const config::Filter &filter) : Filter(filter.material, filter.id, filter.rotation),
  ...
  double BowtieParabolicFilter::calculateIntersectionDistance(const geometry::Ray &ray) const {
    ...
    const double a = ray_transformed.end.x - ray_transformed.start.x;
    const double c = ray_transformed.end.z - ray_transformed.start.z;
    ...
    const double b_0 = 2 * alpha * c * ray_transformed.start.z - a;
    const double c_0 =
        x_f + d_min + alpha * ray_transformed.start.z * ray_transformed.start.z - ray_transformed.start.x;
    ...
    */
    ...
    const double t_4 = (x_4 - ray_transformed.start.x) / (ray_transformed.end.x - ray_transformed.start.x);

    const double y_1 = ray_transformed.start.y + t_1 * (ray_transformed.end.y - ray_transformed.start.y);
    const double y_4 = ray_transformed.start.y + t_4 * (ray_transformed.end.y - ray_transformed.start.y);

    const double z_1 = ray_transformed.start.z + t_1 * (ray_transformed.end.z - ray_transformed.start.z);
    const double z_4 = ray_transformed.start.z + t_4 * (ray_transformed.end.z - ray_transformed.start.z);
    ...
    if (d > 0) {
      ...
      const double x_2 = ray_transformed.start.x + t_2 * (ray_transformed.end.x - ray_transformed.start.x);
      const double x_3 = ray_transformed.start.x + t_3 * (ray_transformed.end.x - ray_transformed.start.x);

      const double y_2 = ray_transformed.start.y + t_2 * (ray_transformed.end.y - ray_transformed.start.y);
      const double y_3 = ray_transformed.start.y + t_3 * (ray_transformed.end.y - ray_transformed.start.y);

      const double z_2 = ray_transformed.start.z + t_2 * (ray_transformed.end.z - ray_transformed.start.z);
      const double z_3 = ray_transformed.start.z + t_3 * (ray_transformed.end.z - ray_transformed.start.z);
